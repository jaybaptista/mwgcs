...
class Stars(ABC):
    def __init__(self, imf_model, stellar_evolution_model, min_mass=0.01, max_mass=150.0):
        self.imf_model = imf_model
        self.stellar_evolution_model = stellar_evolution_model
        self.min_mass = min_mass
        self.max_mass = max_mass

    def sample_stars(self, mass, age=0.0, max_cluster_mass=5e7):
        self.sample = self.imf_model.sample(max_cluster_mass, min_mass=self.min_mass, max_mass=self.max_mass)
        return self.sample

    def get_main_sequence_lifetime(self, mass):
        return self.stellar_evolution_model.main_sequence_lifetime(mass)
...
class IMFModel(ABC):
    ...
    @abstractmethod
    def sample(self, mass):
        pass

class KroupaIMFModel(IMFModel):

    def __init__(self, max_batch=100000, min_batch=10, min_mass=0.01, max_mass=150.0):
        self.max_batch = max_batch
        self.min_batch = min_batch
        self.min_mass = min_mass
        self.max_mass = max_mass

        self.norm = self.normalization(self.min_mass, self.max_mass)

        # compute the CDF
        x = np.linspace(self.min_mass, self.max_mass, 1000)
        cdf = np.cumsum(0.5 * (self.imf(x[1:]) + self.imf(x[:-1])) * np.diff(x))
        cdf /= cdf[-1]
        self.icdf = lambda u: np.interp(u, cdf, x, kind='linear')

    def normalization(self, min_mass, max_mass):
        norm, _ = quad(self.kroupa_imf, min_mass, max_mass)
        return norm
    ...
    def sample(self, mass):
        imf_norm = self.normalization(self.min_mass, self.max_mass)
        imf_avg = quad(lambda m: m * self.imf(m, A=1/imf_norm), self.min_mass, self.max_mass)[0]
        n_draws = mass / imf_avg
        n_draws = np.random.poisson(n_draws)
