...
class Simulation:
    ...
    def getConvergenceRadius(self, snapshot):
        ...
        rho_crit = (3 / (8 * np.pi * c.G.to(u.kpc**3 / u.Msun / u.s**2).value)) * (
            H0
        ) ** 2
...
class MassProfile:
    ...
    def __init__(self, sim, snap, sh_id, boundOnly=False, subsample_frac=0.0):
    ...
    def density_rs(self, bins=50):
        ...
        bin_edges = np.concatenate(
            [[0], [_dist[i[-1]] for i in idx_bins]]  # rightmost bin edge
        )
        ...
        self._rs_bin_count = np.array(
            [
                np.sum((_dist >= bin_edges[i]) & (_dist < bin_edges[i + 1]))
        ...
        rad_bins = np.array_split(
            _dist[indices], bins
        )  # split particles up by their sorted positions
        avg_rad = np.array(
            [np.mean(i) for i in rad_bins]
    ...
    def fit(self, ein=None):
        ...
        radii, density_profile = (
            self.density_rs()
        )  # fit the desnity profile based on rockstar binning
        ...
        popt, pcov = curve_fit(
            ...
            self.bins,
            self.logdata,
            p0=[20, 0.6, 0.18],  # some random values
            maxfev=5000,
        )
...
def DwarfGCMF(mass, M_mean=-7.0, M_sigma=0.7):
...
def assignParticles(mp, mgc, bindEnergy=None):
    ...
    tag_idx = np.random.choice(
        np.arange(len(prob)), size=len(mgc), replace=False, p=prob
    )
    return tag_idx
